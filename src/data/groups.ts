 ...
 */
import { INITIAL_PRODUCTS, INITIAL_FOLDERS, INITIAL_FILTER_GROUPS, DEFAULT_THUMB, type FilterGroup } from '../pages/Products';

/** 상품관리와 동일한 썸네일 정규화 — 업로드 이미지는 유지, 없거나 옛 생성 SVG면 기본 회색+아이콘 */
function normThumb<T extends { thumbUrl?: string }>(p: T): T {
  return (p.thumbUrl && !p.thumbUrl.startsWith('data:image/svg')) ? p : { ...p, thumbUrl: DEFAULT_THUMB };
}
...
export function loadProducts(): PartProduct[] {
  try {
    const raw = JSON.parse(localStorage.getItem('hp3-products-state') ?? '{}');
    if (Array.isArray(raw.products) && raw.products.length) return (raw.products as PartProduct[]).map(normThumb);
  } catch { /* ignore */ }
...
export function loadFolders(): PartFolder[] {
  try {
    const raw = JSON.parse(localStorage.getItem('hp3-products-state') ?? '{}');
    if (Array.isArray(raw.folders) && raw.folders.length) return raw.folders as PartFolder[];
  } catch { /* ignore */ }
...
export function loadSwapState(): SwapState {
  try {
    ...
    if (raw && raw.__v === SWAP_VERSION && Array.isArray(raw.groups)) {
      ...
      return { groups: raw.groups, folders: raw.folders ?? {}, folderModes: raw.folderModes ?? {}, items: raw.items ?? {}, groupRefs: raw.groupRefs ?? {}, styles: raw.styles ?? [], categories, styleCategories: raw.styleCategories ?? SEED.styleCategories };
    }
  } catch { /* ignore */ }
  return SEED;
}
